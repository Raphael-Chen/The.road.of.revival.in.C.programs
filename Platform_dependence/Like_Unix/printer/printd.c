...
int main(int argc, char *argv[])
{
    ...
    err = getaddrlist(host, "print", &ailist);
    if ( err != 0 )
    {
...
void *client_thread(void *arg)
{
	 ...
	 */
    n = treadn(sockfd, &req, sizeof(struct printreq), 10);
...
void add_worker(pthread_t tid, int sockfd)
{
    ...
    wtp = malloc(sizeof(struct worker_thread));
    if ( wtp == NULL)
    {
...
void *printer_thread(void *arg)
{
    ...
    for (;;)
    {
        ...
        sockfd = connect_retry(AF_INET, SOCK_STREAM, 0, 
                                printer->ai_addr, printer->ai_addrlen);
        if ( sockfd < 0)
        {
