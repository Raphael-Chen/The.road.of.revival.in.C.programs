...
int main(int argc, char *argv[])
{
    ...
    {
        sfd = connect_retry(AF_INET, SOCK_STREAM, 0, aip->ai_addr, aip->ai_addrlen);
        if ( sfd < 0 )
        {
            err = errno;
        }
                 ...
                 int text)
{
	 ...
	 * Read the response.
	 */
    nr = readn(sockfd, &res, sizeof(struct printresp));
    if ( nr != sizeof(struct printresp) )
